...
func NewErrAALNotSatisfied(redirectTo string) *ErrAALNotSatisfied {
	return &ErrAALNotSatisfied{
		...
		DefaultError: herodot.ErrForbidden.
			WithError("session does not fulfill the requested AAL").
			WithDetail("redirect_browser_to", redirectTo).
			WithReason("An active session was found but it does not fulfill the requested Authenticator Assurance Level. You might be able to resolve this by signing in with 2FA."),
	}
}
