...
func (m *RegistryDefault) PostSettingsPrePersistHooks(ctx context.Context, settingsType string) (b []settings.PostHookPrePersistExecutor) {
	for _, v := range m.getHooks(settingsType, m.Config(ctx).SelfServiceFlowSettingsAfterPrePersistHooks(settingsType)) {
...
func (m *RegistryDefault) PostSettingsPostPersistHooks(ctx context.Context, settingsType string) (b []settings.PostHookPostPersistExecutor) {
	...
	for _, v := range m.getHooks(settingsType, m.Config(ctx).SelfServiceFlowSettingsAfterPostPersistHooks(settingsType)) {
	...
	if len(b) == initialHookCount {
		...
		for _, v := range m.getHooks(config.HookGlobal, m.Config(ctx).SelfServiceFlowSettingsAfterPostPersistHooks(config.HookGlobal)) {
			if hook, ok := v.(settings.PostHookPostPersistExecutor); ok {
				b = append(b, hook)
			}
		}
